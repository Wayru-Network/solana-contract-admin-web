...
export const MintTokensForm = ({ form, provider, onSubmit, mintKeyStatus, handleKeypairChange, mintKeypairAddress, isCheckingMintKeyStatus }: MintTokensFormProps) => {
    ...
    return (
      <Form form={form} layout="vertical" onFinish={onSubmit}>
                ...
                { min: 20, message: "URL must be at least 10 characters long" },
                {
                  validator: (_, value) => {
                    ...
                    return /^https:\/\/ipfs/.test(value) 
                      ? Promise.resolve() 
                      : Promise.reject(new Error('URL must start with "https://ipfs"'));
                  },
                },
              ]}
            >
              ...
              rules={[
                {
                  validator: (_, value) => {
                    ...
                    try {
                      ...
                      return Promise.reject(new Error("Please enter a valid Solana address"));
                    }
                  },
                },
              ]}
        ...
        <Button
          ...
          block
        >
          {mintKeyStatus?.exists && mintKeyStatus?.isToken ? "Token Already Exists" : "Mint"}
        </Button>
      </Form>
    );
  };
